...
class EmployeeTests {
    ...
    private val user = Employee(1, "some user")

    @Test
    fun `can serialize User with Kotlin serialization`() {
        // serializing objects
        val jsonData = jsonSerialzation.stringify(Employee.serializer(), user)
        assertEquals("""{"id":1,"name":"some user"}""", jsonData)

        // serializing lists
        val jsonList = jsonSerialzation.stringify(Employee.serializer().list, listOf(user))
        assertEquals("""[{"id":1,"name":"some user"}]""", jsonList)

        // parsing data back
        val obj: Employee = jsonSerialzation.parse(Employee.serializer(), """{"id":1,"name":"some user"}""")
    ...
    fun `failed deserialization should make it clear what went wrong`() {
        val ex = assertThrows(JsonDecodingException::class.java) {
            jsonSerialzation.parse(Employee.serializer(), """{"id":1ABC,"name":"some user"}""") }
    ...
    fun `failed deserialization should make it clear what went wrong, empty id`() {
        val ex = assertThrows(JsonDecodingException::class.java) {
            jsonSerialzation.parse(Employee.serializer(), """{"id":,"name":"some user"}""") }
    ...
    fun `failed deserialization should make it clear what went wrong, negative id`() {
        val ex = assertThrows(AssertionError::class.java) {
            jsonSerialzation.parse(Employee.serializer(), """{"id":-1,"name":"some user"}""") }
    ...
    fun `failed deserialization should make it clear what went wrong, too large id`() {
        val ex = assertThrows(JsonDecodingException::class.java) {
            jsonSerialzation.parse(Employee.serializer(), """{"id":123456789012345678901234567890,"name":"some user"}""") }
